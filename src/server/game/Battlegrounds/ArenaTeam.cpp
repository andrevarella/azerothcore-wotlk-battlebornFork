...
void ArenaTeam::SaveToDB(bool forceMemberSave)
{
    ...
    //{
        //LOG_ERROR("teste1", "SaveToDB - TeamID >= 4293918720");
    //    SaveSoloDB(this);
    //    return;
    //}

    //LOG_ERROR("solo3v3", "SaveToDB - TeamID < 4293918720");
...
void ArenaTeam::SaveSoloDB(ArenaTeam* team)
{
    ...
    for (auto const& itr : team->GetMembers())
    {
        ...
        {
            if (itrMgr.first < 0xFFF00000 && itrMgr.second->GetCaptain() == itr.Guid && itrMgr.second->GetType() == 4) // itrMgr.second->GetType() == 4 || itrMgr.second->GetType() == 5
            {
            ...
            {
                //LOG_ERROR("solo3v3", "SaveSoloDB - team not found");
            }
        }
...
std::unordered_map<uint8, uint8> ArenaTeam::ArenaReqPlayersForType =
{
    { ARENA_TYPE_2v2, 4},
    { ARENA_TYPE_3v3, 6},
    { ARENA_TYPE_5v5, 10}, // n precisa aparentemente
};
