...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit)
    {
        ...
        //                                                                                          Intervene Trigger               Pestilence                  Chaos Bane                 Distract (vs Mage inv)
        if (m_caster->_IsValidAttackTarget(unit, m_spellInfo) && /*Intervene Trigger*/ m_spellInfo->Id != 59667 && m_spellInfo->Id != 50842 && m_spellInfo->Id != 71904 && m_spellInfo->Id != 1725)
        {
    ...
    //                                                                                                                                                                                                Pestilence              Chaos Bane (nao quebram stealth)
    if (m_caster != unit && m_caster->IsHostileTo(unit) && !m_spellInfo->IsPositive() && !m_triggeredByAuraSpell && !m_spellInfo->HasAttribute(SPELL_ATTR0_CU_DONT_BREAK_STEALTH) && m_spellInfo->Id != 50842 && m_spellInfo->Id != 71904)
    {
        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
                    ...
                    /*
                    // Se tiver a aura 83289, dots/hots nao sao reduzidos pela haste
                    if (!m_caster->HasSpell(83289) && (m_originalCaster->HasAuraTypeWithAffectMask(SPELL_AURA_PERIODIC_HASTE, aurSpellInfo) || m_spellInfo->HasAttribute(SPELL_ATTR5_SPELL_HASTE_AFFECTS_PERIODIC)))
                    {
...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    if ((m_spellInfo->IsChanneled() || m_casttime) && m_caster->GetTypeId() == TYPEID_PLAYER && m_caster->isMoving() && m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT && !IsTriggered()/*&& m_spellInfo->Id != 83292*/)
    {
...
uint32 Spell::GetCCDelay(SpellInfo const* _spell)
{
    ...
    const uint32 delayForInstantSpells = 75;

    // CCDelay for spell with auras
    uint8 CCDArraySize = 7;
    AuraType auraWithCCD[] =
    {
        SPELL_AURA_MOD_STUN,     // 115
        SPELL_AURA_MOD_CONFUSE,  // 115
        SPELL_AURA_MOD_FEAR,     // 115
        SPELL_AURA_MOD_SILENCE,  // 115
        SPELL_AURA_MOD_DISARM,   // 70
        SPELL_AURA_MOD_ROOT,     // 70
        SPELL_AURA_MOD_POSSESS   // 115
    };
    ...
    // Cases for specific race/class spells
    switch (_spell->Id)
    {
        ...
        case 6213:
        case 6215:
        case 5484: // Warlock - Howl of Terror
        case 17928:
            return delayForFears;
        case 676: // Warrior - Disarm
        case 51722: // Rogue - Dismantle
        ...
        case 19386: // Hunter - Wyvern Sting
        case 24132:
        case 24133:
        case 27068:
        case 49011:
        ...
        case 9485:
        case 9484:
        case 49203: // DK - Hungering Cold
        case 51514: // Shaman - Hex
        case 42950: // Mage - Dragon Breath
        case 42949:
        case 33043:
        case 33042:
        case 33041:
        ...
        case 36554: // Rogue - Shadowstep
        case 25046: // Racial - Blood Elf
        case 47486: // Warrior Mortal Strike
        case 49050: // Aimed Shot
        case 59159: // Thunderstorm
        case 49576: // DK - Death Grip
        case 45334: // Druid - Feral charge - Bear
        case 49376: // Druid - Feral charge - Cat
        ...
        case 12494: // Mage - Talent - Frostbite
        case 122:   // Mage - Frostnova
        case 865:
        case 6131:
        case 10230:
        case 27088:
