...
static uint32 copseReclaimDelay[MAX_DEATH_COUNT] = { 15, 30, 60 };
...
void Player::ApplyEquipCooldown(Item* pItem)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (spellData.SpellTrigger == ITEM_SPELLTRIGGER_ON_EQUIP)
        {
            ...
            AddSpellCooldown(spellData.SpellId, uint32(-1), 15 * IN_MILLISECONDS);
            continue;
        }
        ...
        AddSpellCooldown(spellData.SpellId, pItem->GetEntry(), 15 * IN_MILLISECONDS, true, true);
...
bool Player::InArenaQueue() const
{
    for (uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        if (_BgBattlegroundQueueID[i].bgQueueTypeId == BATTLEGROUND_QUEUE_2v2 || _BgBattlegroundQueueID[i].bgQueueTypeId == BATTLEGROUND_QUEUE_3v3)
        {
            return true;
        }
    }
