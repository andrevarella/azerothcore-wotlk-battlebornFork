...
float Unit::GetEffectiveResistChance(Unit const* owner, SpellSchoolMask schoolMask, Unit const* victim)
{
    ...
    static float const BOSS_RESISTANCE_CONSTANT = 999999.0f;
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    if (canDodge)
    {
        ...
        /*
        // Verifica se ambos atacante e alvo são jogadores
        if (GetTypeId() == TYPEID_PLAYER && victim->GetTypeId() == TYPEID_PLAYER)
        {
            // Reduz a chance de dodge em 50%
            dodgeChance = int32(dodgeChance * 0.5);
            //dodgeChance -= 5
        } */
...
void Unit::RemoveArenaAuras()
{
    ...
    RemoveAppliedAuras([](AuraApplication const* aurApp)
    {
        ...
        || aura->GetSpellInfo()->HasAttribute(SPELL_ATTR4_NOT_IN_ARENA_OR_RATED_BATTLEGROUND);
    });
}
        ...
        case SPELLFAMILY_GENERIC:
            {
                switch (dummySpell->Id)
                {
                    ...
                    {
                        if (GetTypeId() != TYPEID_PLAYER || HasSpellCooldown(83095))
                            return false;
...
void Unit::SetInCombatState(bool PvP, Unit* enemy, uint32 duration)
{
    ...
    if (PvP)
        m_CombatTimer = std::max<uint32>(GetCombatTimer(), std::max<uint32>(5550, duration));
    else if (duration)
